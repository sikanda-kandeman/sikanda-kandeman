...
function openDialog(modal, initialFocus) {
  if (!modal) return;
  _dialogReturnFocus.set(modal.id, document.activeElement);
  document.getElementById('admin-shell')?.setAttribute('inert', '');
  document.body.classList.add('modal-open');
  modal.classList.add('open');
  modal.setAttribute('aria-hidden', 'false');
  requestAnimationFrame(() => (initialFocus || modal.querySelector(DIALOG_FOCUSABLE) || modal).focus());
}
...
function closeDialog(modal) {
  ...
  document.getElementById('admin-shell')?.removeAttribute('inert');
  document.body.classList.remove('modal-open');
  const returnFocus = _dialogReturnFocus.get(modal.id);
  _dialogReturnFocus.delete(modal.id);
  if (returnFocus && typeof returnFocus.focus === 'function') returnFocus.focus();
}

function trapDialogFocus(event, modal) {
  const focusable = Array.from(modal.querySelectorAll(DIALOG_FOCUSABLE))
...
function openConfirm(title, desc, okLabel, cb) {
  ...
  openDialog(modal, document.getElementById('confirm-ok-btn'));
}
...
function openSidebar() {
  ...
  document.querySelector('.sb-item.active')?.focus();
}
...
document.addEventListener('keydown', e => {
  const openModal = document.querySelector('.modal-overlay.open, .preview-modal.open');
  if (e.key === 'Tab' && openModal) {
    trapDialogFocus(e, openModal);
    return;
  }
  if (e.key !== 'Escape') return;
  if (document.getElementById('confirm-modal')?.classList.contains('open')) closeConfirm();
  else if (document.getElementById('password-modal')?.classList.contains('open')) closePasswordModal();
...
async function doLogout() {
  await window.SikandaAdminAuth?.logout();
}
...
function resetInactivityTimer() {
  ...
  inactivityTimer = setTimeout(() => {
    window.SikandaAdminAuth?.logout('Sesi berakhir karena tidak aktif selama 30 menit.');
  }, 30*60*1000);
}

async function startAdminApp(user) {
...
function ubahPassword() {
  ...
  form.reset();
  showPasswordError('');
  openDialog(document.getElementById('password-modal'), document.getElementById('password-current'));
}
...
async function handlePasswordChange(event) {
  ...
  if (newPassword !== confirmation) {
    showPasswordError('Konfirmasi password baru tidak cocok.');
    return;
  }
  ...
  saveButton.textContent = 'Memverifikasi...';

  try {
    const { data: { user }, error: userError } = await sb.auth.getUser();
    if (userError || !user?.email) throw userError || new Error('Sesi admin tidak tersedia.');
    ...
    if (passwordError) {
      showPasswordError('Password saat ini tidak benar.');
      return;
    }
    ...
    showPasswordError('Password belum dapat diubah. Periksa koneksi lalu coba lagi.');
  } finally {
...
window.SikandaAdminApp = Object.freeze({ start: startAdminApp });
